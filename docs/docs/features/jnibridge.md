---
sidebar_position: 4
sidebar_label: "JNI Bridge"
title: "JNI Bridge Feature: Native C++ Integration via JNI"
description: "Generate JNI bridge classes for integrating the Java/Android API with native C++ code, such as Unreal Engine."
keywords: [jni, jni bridge, native, c++, unreal engine, apigear, android native]
---
...
:::info
This feature requires the [api](api.md), [stubs](stubs.md), and [android](android.md) features. It provides both a JNI-backed service implementation and a JNI-forwarding client.
:::
...
- **JNI Client** — wraps the `HelloClient` (from the [android](android.md) feature) and forwards all events to native code via JNI callbacks. Use this when native code needs to consume the API as a client.

```mermaid
graph TD
    subgraph NativeCode[" Native C++ "]
        NativeConsumer["Native Consumer"]
        NativeImpl["Native Implementation"]
    end

    subgraph JNIBridge[" JNI Bridge "]
        JniClient["HelloJniClient<br/>(JNI callbacks)"]
        JniService["HelloJniService<br/>(native methods)"]
    end

    subgraph AndroidIPC[" Android Service/Client "]
        Client["HelloClient"]
        ServiceAdapter["HelloServiceAdapter"]
    end

    NativeImpl <-->|JNI calls| JniService
    JniService -->|backend for| ServiceAdapter
    Client -->|Messenger IPC| ServiceAdapter
    JniClient -->|wraps| Client
    NativeConsumer <-->|JNI callbacks| JniClient
```
...
| `nativeAsyncOperationFailed(String, String, int)` | Java → C++ | Forward RPC failure to native |
