...
title: "Android Feature: Service/Client IPC via Messenger"
description: "Generate Android Messenger-based service/client architecture for cross-process and cross-app API communication."
keywords: [android service, android messenger, ipc, android client, apigear, cross-process]
---
...
graph LR
    subgraph ClientApp[" Client App "]
        ...
        Client["HelloClient<br/>(implements IHello)"]
    end

    subgraph IPC[" Messenger IPC "]
        Msg["Messages &<br/>Parcelables"]
    end

    subgraph ServiceApp[" Service App "]
        Adapter["HelloServiceAdapter<br/>(Android Service)"]
        Backend["Backend<br/>(Stubs or JNI)"]
    end

    App -->|uses| Client
    Client -->|sends| Msg
    Msg -->|receives| Adapter
    Adapter -->|delegates to| Backend
    Backend -.->|events| Adapter
    Adapter -.->|notifies| Msg
    Msg -.->|receives| Client
    Client -.->|notifies| App
```
...
Singleton that creates a `HelloService` (from [stubs](stubs.md)) as the backend:
...
:::caution Extern types and class loaders
