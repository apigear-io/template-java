		...
		public void handleMessage(Message msg)
		{
			...
			switch (msgType)
			{
				...
				case RPC_FuncVoidReq: {
					...
					if (backend == null || !backend._isReady()) {
						...
						if (replyTo != null) {
							try {
								...
								Log.e(TAG, "failed to send funcVoid not-ready response: " + e);
							}
						}
						break;
					}
					...
					backend.funcVoidAsync().whenComplete((Void unused, Throwable error) -> {
						Message respMsg = new Message();
						respMsg.what = NoPropertiesInterfaceMessageType.RPC_FuncVoidResp.getValue();
						Bundle resp_data = new Bundle();
						resp_data.putInt("callId", callId);
						...
						if (error != null) {
							...
							Log.d(TAG, "funcVoid exception details", cause);
							resp_data.putBoolean("error", true);
