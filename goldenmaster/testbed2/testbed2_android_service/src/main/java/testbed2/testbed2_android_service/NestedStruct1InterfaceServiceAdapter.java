...
public class NestedStruct1InterfaceServiceAdapter extends Service
{
	...
	class IncomingHandler extends Handler implements INestedStruct1InterfaceEventListener
	{
        ...
        data.setClassLoader(NestedStruct1Parcelable.class.getClassLoader());
					...
					if (backend == null || !backend._isReady()) {
						...
						if (replyTo != null) {
							try {
								...
								Log.e(TAG, "failed to send funcNoReturnValue not-ready response: " + e);
							}
						}
						break;
					}
					...
					backend.funcNoReturnValueAsync(param1).whenComplete((Void unused, Throwable error) -> {
						Message respMsg = new Message();
						respMsg.what = NestedStruct1InterfaceMessageType.RPC_FuncNoReturnValueResp.getValue();
						Bundle resp_data = new Bundle();
						resp_data.putInt("callId", callId);
						...
						if (error != null) {
							...
							Log.d(TAG, "funcNoReturnValue exception details", cause);
							resp_data.putBoolean("error", true);
				...
				case RPC_FuncNoParamsReq: {
					...
					if (backend == null || !backend._isReady()) {
						...
						if (replyTo != null) {
							try {
								...
								Log.e(TAG, "failed to send funcNoParams not-ready response: " + e);
							}
						}
						break;
					}
					...
					// Async dispatch — returns immediately, looper is free
					backend.funcNoParamsAsync().whenComplete((Object rawResult, Throwable error) -> {
						Message respMsg = new Message();
						respMsg.what = NestedStruct1InterfaceMessageType.RPC_FuncNoParamsResp.getValue();
						Bundle resp_data = new Bundle();
						resp_data.putInt("callId", callId);
						...
						if (error != null) {
							...
							Log.d(TAG, "funcNoParams exception details", cause);
							resp_data.putBoolean("error", true);
							resp_data.putString("errorMessage", errorMessage);
							int errorCode;
							if (cause instanceof RemoteOperationException) {
